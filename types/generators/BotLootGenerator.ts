...
export class BotLootGenerator
{
        ...
        isPmc = false): void
    {
        ...
        if (pool.length)
        {
            ...
            for (let i = 0; i < totalItemCount; i++)
            {
                ...
                // Fill ammo box
                if (this.isAmmoBox(itemToAddTemplate._props))
                {
                    itemsToAdd.push(this.createAmmoForAmmoBox(id, itemToAddTemplate._props));
                }

                if (this.isAmmoBox(itemToAddTemplate._props))
                {
                    const ammoBoxMaxCartridgeCount = itemToAddTemplate._props.StackSlots[0]._max_count;
                    const cartridgeTpl = itemToAddTemplate._props.StackSlots[0]._props.filters[0].Filter[0];
                    ...
                    while (currentStoredCartridgeCount < ammoBoxMaxCartridgeCount)
                    {
                        ...
                        itemsToAdd.push(this.itemHelper.createCartridges(id, cartridgeTpl, cartridgeCountToAdd, location));
    ...
    protected createAmmoForAmmoBox(parentId: string, props: Props): Item 
    {
        return {
            ...
            _tpl: this.randomUtil.getArrayValue(props.StackSlots[0]._props.filters[0].Filter),
            parentId: parentId,
            slotId: "cartridges",

...
export class RepeatableQuestController
{
        ...
        for (const repeatableConfig of this.questConfig.repeatableQuests)
        {
                ...
                && pmcData.Info.Level >= repeatableConfig.minPlayerLevel || repeatableConfig.side === "Scav" && scavQuestUnlocked)
            {
                if (time > currentRepeatableType.endTime - 1)
                {
                    ...
                    const questTypePool = this.generateQuestPool(repeatableConfig);
    ...
    ): IElimination
    {
        ...
        let isDistanceRequirementAllowed = !repeatableConfig.questConfig.Elimination.distLocationBlacklist.includes(locationKey);
        ...
        if (targetsConfig.data(targetKey).isBoss)
        {
            ...
            const allowedSpawns = thisBossSpawns.filter(x => !repeatableConfig.questConfig.Elimination.distLocationBlacklist.includes(x.Id));
     ...
     */
    public generateQuestPool(repeatableConfig: IRepeatableQuestConfig): IQuestTypePool
    {
        ...
        const targetsConfig = this.probabilityObjectArray(repeatableConfig.questConfig.Elimination.targets);
